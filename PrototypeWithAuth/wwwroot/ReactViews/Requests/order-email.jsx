...
export default function OrderEmail(props) {
    ...
    var colspan = 4;
    var requestText = props.viewModel.IsPriceQuote ? "We would like to get a price quote for the following items:" : "";

    const priceRows = () => {
        var total = 0;
        var vat = 0;
        var requests = [];
        props.viewModel.Requests?.length > 0 ? requests = props.viewModel.Requests : props.tempRequestList.TempRequestViewModels.map(tempRequest => {
        ...
        var unitPrices = requests.map(request => {
            ...
            return (
                <tr key={request.RequestID}>
                    ...
                    <td>{request.Unit}</td>
                    {!props.viewModel.IsPriceQuote && <td className="">{currency + "" + ((cost || 0) / Number(request.Unit).toFixed(2))}</td>}
                    {!props.viewModel.IsPriceQuote && <td className="">{(request.ParentQuote?.Discount || 0) + "%"}</td>}
                    {!props.viewModel.IsPriceQuote && <td className="">{currency + "" + Number(cost).toFixed(2)}</td>}
                </tr>
            )
        ...
        <div id="orderEmail">
            <header className="page-header">
                <div className="row">
                    ...
                    <div className="col-md-4 offset-2 pt-3 company-info">
            ...
            <div className="row">
                <div className="col-6">
                    ...
                    {/*    :*/}
                    <span style={{ fontWeight: "500", fontSize: "1rem" }}>{"Purchase Order: " + props.viewModel.ParentRequest?.OrderNumber}</span>
                </div>
                    ...
                    <div className="col-6 offset-2">
                        <div className="note-message p-4 pl-5">
                            ...
                            <div>
                                ...
                                <span className="text">{props.viewModel.ParentRequest?.NoteToSupplier}</span>
                            </div>
                        </div>
                    </div>
                }
                ...
                <table className="table confirm-table">
                    <thead style={{ backgroundColor: "#D0001C", color: "#ffffff" }}>
                        <tr>
                            ...
                            {!props.viewModel.IsPriceQuote && <th style={{ width: "16%" }}>Price</th>}
                            {!props.viewModel.IsPriceQuote && <th style={{ width: "15%" }}>Discount</th>}
                            {!props.viewModel.IsPriceQuote && <th style={{ width: "16%" }}>Total</th>}
            ...
            {props.viewModel.PaymentStatus &&
                <div className="row">
                    <div className="col-6">
                        ...
                        <span className="text">
                            {props.viewModel.PaymentStatusID == 5 &&
                                (firstRequest.Installments + " ")}
                            {props.viewModel.PaymentStatus.PaymentStatusDescription}
                        </span>{/*<br />*/}
                    </div>
                </div>
            }
