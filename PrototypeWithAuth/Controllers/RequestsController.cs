...
namespace PrototypeWithAuth.Controllers
{
    public class RequestsController : SharedController
    {
            ...
            switch (requestIndexObject.PageType)
            {
                ...
                case AppUtility.PageTypeEnum.AccountingNotifications:
                    ...
                    (AppUtility.IndexTableTypes.AccountingNotifications, r, r.Product, r.Product.Vendor, r.Product.ProductSubcategory,
                        r.Product.ProductSubcategory.ParentCategory, r.Product.UnitType, r.Product.SubUnitType, r.Product.SubSubUnitType, requestIndexObject, iconList, defaultImage, r.ParentRequest, checkboxString, _requestsProc.ReadOne( new List<Expression<Func<Request, bool>>> { pr => pr.ParentRequestID == pr.ParentRequestID && r.ProductID == pr.ProductID && r.RequestStatusID == 2 }, null).FirstOrDefault())
                    {
        ...
        protected async Task<TempRequestViewModel> AddItemAccordingToOrderType(Request newRequest, AppUtility.OrderTypeEnum OrderTypeEnum, bool isInBudget, RequestItemViewModel requestItemViewModel, int requestNum = 1, ReceivedModalVisualViewModel receivedModalVisualViewModel = null)
        {
                ...
                try
                {
                    ...
                    if (Validator.TryValidateObject(request, context, results, true))
                    {
                        ...
                        if (requestItemViewModel.Comments != null)
                        {
                           ...
                           await SaveRequestProductCommentsFunctionAsync(requestItemViewModel.Comments, request, currentUser);
                        }
        ...
        private async Task SaveRequestProductCommentsFunctionAsync(List<CommentBase> comments, Request request, Employee currentUser)
        {
            //save comments
            await _requestCommentsProc.UpdateAsync((List<RequestComment>)comments.Where(c => c.CommentTypeID==1), request.RequestID, currentUser.Id);
            await _productCommentsProc.UpdateAsync((List<ProductComment>)comments.Where(c => c.CommentTypeID==2), request.ProductID, currentUser.Id);
        }
                        ...
                        try
                        {
                            ...
                            if (wasSent)
                            {
                                ...
                                for (int tr = 0; tr < deserializedTempRequestListViewModel.TempRequestViewModels.Count(); tr++)
                                {
                                    ...
                                    {
                                        await SaveRequestProductCommentsFunctionAsync(tempRequest.Comments, tempRequest.Request, currentUser);
                                    }
...
.Include(r => r.Product).ThenInclude(p => p.Vendor).Include(r => r.Product.ProductSubcategory).ThenInclude(ps => ps.ParentCategory).AsNoTracking().FirstOrDefault();
        ...
        {
            var requests = _requestsProc.Read(new List<Expression<Func<Request, bool>>> { r => r.RequestStatusID != 7 && r.Payments.Where(p => !p.IsPaid).Count() > 0 },
                  new List<ComplexIncludes<Request, ModelBase>> {
                    new ComplexIncludes<Request, ModelBase> { Include = r => r.Product, ThenInclude= new ComplexIncludes<ModelBase, ModelBase> { Include =p => ((Product)p).Vendor } },
                        ...
                        try
                        {
                            foreach (var tempRequestViewModel in deserializedTempRequestListViewModel.TempRequestViewModels)
                            {
                                ...
                                if (tempRequestViewModel.Comments != null)
                                {
                                    ...
                                    await SaveRequestProductCommentsFunctionAsync(tempRequestViewModel.Comments, tempRequestViewModel.Request, currentUser);
                                }
        ...
        {
            await _requestListRequestsProc.MoveList(moveListViewModel.Request.RequestID, moveListViewModel.NewListID, moveListViewModel.PreviousListID);
        ...
        {
            await _requestListsProc.UpdateAsync(listSettings, _userManager.GetUserId(User));
        ...
        private async Task<ListSettingsViewModel> GetListSettingsInfoAsync(int selectedListID, AppUtility.SidebarEnum sidebarType)
        {
            ...
            viewModel.SharedUsers = _shareRequestListsProc.Read( new List<Expression<Func<ShareRequestList, bool>>> { l => l.RequestListID == selectedListID }, new List<ComplexIncludes<ShareRequestList, ModelBase>> { new ComplexIncludes<ShareRequestList, ModelBase> { Include = l => l.ToApplicationUser } }).Select(
        ...
        private async  Task<ListSettingsViewModel> GetSharedListSettingsAsync(int selectedListID)
        {
            ...
            var defaultList = await _shareRequestListsProc.ReadOneAsync(new List<Expression<Func<ShareRequestList, bool>>> { srl => srl.RequestListID == selectedListID && srl.ToApplicationUserID == _userManager.GetUserId(User) },
        ...
        {
            var viewModel = _requestListsProc.ReadOneAsync(new List<Expression<Func<RequestList, bool>>> { rl => rl.ListID == listID });
        ...
        {
             await _requestListsProc.DeleteAsync(deleteList);
